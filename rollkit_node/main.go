...
func init() {
	...
	flag.StringVar(&namespaceId, "rollmint.namespace_id", "0000000000000000", "namespace id to use")
	flag.Uint64Var(&daStartHeight, "rollmint.da_start_height", 0, "height to start at when querying blocks")
	flag.BoolVar(&aggregator, "rollmint.aggregator", true, "run node on aggregator mode or not")
	flag.StringVar(&daLayer, "rollmint.da_layer", "celestia", "data availability layer to use")
	flag.StringVar(&daConfig, "rollmint.da_config", `{"base_url":"http://localhost:26659","timeout":60000000000,"gas_limit":6000000,"fee":6000}`, "configuration to use for the data availability layer")
